...
const Tabledetails = (props) => {
  ...
  const fetchData = async () => {
    ...
    http.post('content', formdata).then((res) => {
  ...
  const DELETE = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        await http.post('deleteContent', formData).then((res) => {
            ...
            <td style={{ paddingTop: '30px' }}>
              <span className="d-flex align-items-center gap-2">
                <button type="button" onClick={() =>
                  navigate("/update-content", { state: res })
                }
                  className="py-2 px-3 btn-grey border-0 rounded-3">
                  <i className="fa fa-edit"></i>
                </button>
                <button type="button" onClick={() => DELETE(res.id)} className="py-2 px-3 btn-grey border-0 rounded-3">
